...
int speed = 1;
void move_mouse(int scancode, int state){
  xdo = xdo_new(NULL);
  if (state != 0){
    if (scancode == 35){ // case h
      xdo_move_mouse_relative(xdo, (-20*speed), 0); // move left
    }
    if (scancode == 36){ // case j
      xdo_move_mouse_relative(xdo, 0, (20*speed)); // move down
    }
    if (scancode == 37){ //case k
      xdo_move_mouse_relative(xdo, 0, (-20*speed)); // move up
    }
    if (scancode == 38){ //case l
      xdo_move_mouse_relative(xdo, (20*speed), 0); // move right
    }
    if (scancode == 30){ // case a
      speed = 1;
      printf("speed %d\n", speed);
    }
    if (scancode == 31){ // case s 
      speed = 2;
      printf("speed %d\n", speed);
    }
    if (scancode == 32){ // case d
      speed = 4;
      printf("speed %d\n", speed);
    }
    if (scancode == 33){ // case f
      speed = 8;
      printf("speed %d\n", speed);
    }
    if (scancode == 46){ // case c
  ...
  while(true){
    ...
    if (event.type == EV_KEY && event.value >= 0 && event.value <= 2){
      ...
      if (mode_normal == false){
          ...
          move_mouse(event.code, event.value);
      }
    }
  }
  return 0;
